...
bool is_valid_track_folder_name(const std::string& folder, const std::string& date) {
    if (folder.substr(0, 8) != date) return false;
    std::regex pattern(R"(^\d{8}_Track\d{2}$)");
    return std::regex_match(folder, pattern);
}

bool is_valid_text_file_name(const std::string& filename, const std::string& prefix) {
    if (filename.substr(0, 16) != prefix) return false;
    std::regex pattern(R"(^\d{8}_Track\d{2}_C1_EO.txt$)");
    return std::regex_match(filename, pattern);
}

std::vector<TrackData> parse_file(const std::string& filepath) {
...
void interpolate(std::vector<TrackData>& data) {
...
void write_file(const std::string& filepath, const std::vector<TrackData>& data) {
...
void process_folder(const std::filesystem::path& path, const std::string& prefix) {
